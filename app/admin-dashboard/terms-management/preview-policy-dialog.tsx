...
export function PreviewPolicyDialog({ open, onOpenChange, policies }: PreviewPolicyDialogProps) {
    ...
    const nextSection = () => {
        if (currentSection < policies.length - 1) {
            setCurrentSection(currentSection + 1)
        }
    }

    const isLastSection = currentSection === policies.length - 1
            ...
            <DialogContent className="max-w-4xl h-[80vh] flex flex-col">
                <DialogHeader>
                    <DialogTitle className="flex justify-between items-center">
                        ...
                        <span className="text-sm font-normal text-muted-foreground">
                            Section {currentSection + 1} of {policies.length}
                        </span>
                    </DialogTitle>
                    <DialogDescription>Preview how your privacy policy will appear to users.</DialogDescription>
                </DialogHeader>
                <ScrollArea className="flex-grow pr-4 mt-4">
                    <AnimatePresence mode="wait">
                        ...
                        >
                            <h2 className="text-2xl font-bold mb-4">{policies[currentSection]?.title || "No title"}</h2>
                            <p className="whitespace-pre-wrap leading-relaxed text-lg">
                                {policies[currentSection]?.content || "No content"}
                            </p>
                        </motion.div>
                    </AnimatePresence>
                </ScrollArea>
                <div className="flex flex-col sm:flex-row justify-between items-center mt-4 gap-4 p-4">
                    <div className="flex items-center space-x-2 sm:order-1">
                        ...
                        />
                        <Label htmlFor="privacy" className={`${!isLastSection ? "text-gray-400" : ""} text-sm`}>
                ...
                <div className="relative w-full mt-4">
                    <motion.div
                        ...
                        initial={{ width: 0 }}
                        animate={{
                            width: `${((currentSection + 1) / policies.length) * 100}%`,
                        }}
